...
class Dis_content(nn.Module):
  def __init__(self):
    ...
    model = []
    model += [LeakyReLUConv2d(256, 256, kernel_size=4, stride=1, padding=1, norm='Instance')]
    model += [LeakyReLUConv2d(256, 256, kernel_size=4, stride=1, padding=1, norm='Instance')]
    model += [LeakyReLUConv2d(256, 256, kernel_size=4, stride=1, padding=1, norm='Instance')]
...
class G(nn.Module):
  def __init__(self, output_dim_a, output_dim_b, nz):
    ...
    self.decA5_2 = decA5_2
    self.decA6 = nn.Sequential(*decA6)
    self.decA6_2 = nn.Sequential(*decA6_2)
    ...
    decB5_2 = ReLUINSConvTranspose2d(tch//2, tch//2, kernel_size=3, stride=2, padding=1, output_padding=1)
    tch = tch//2
    decB6 = []
    ...
    tch = tch//2
    decB6_2 = [INSResBlock(tch, tch), INSResBlock(tch, tch), ReLUINSConvTranspose2d(tch, tch//2, kernel_size=3, stride=2, padding=1, output_padding=1)]
    decB6 += [nn.ConvTranspose2d(tch, tch, kernel_size=1, stride=1, padding=0)]
    decB6_2 += [nn.ConvTranspose2d(tch//2, output_dim_b, kernel_size=1, stride=1, padding=0)]
    decB6_2 += [nn.Tanh()]
    self.decB5 = decB5
    self.decB5_1 = nn.Sequential(*decB5_1)
    self.decB5_2 = decB5_2
  ...
  def forward_b(self, x, z):
    ...
    out5_1 = self.decB5_2(out5)
    out5_img = self.decB5_1(out5_1)
    out6 = self.decB6(out5_1)
    out6_1 = self.decB6_2(out6)
    return out6_1, out5_img
